...
def format_pile(stones_in_pile: int) -> str:
  ...
  """
  max_stones_in_row = math.ceil(math.sqrt(stones_in_pile))
  total_full_rows = math.floor(stones_in_pile/max_stones_in_row)
  rows = 0
  remainder = stones_in_pile % max_stones_in_row
  picture = "\n"
  while rows < total_full_rows:
    stones_in_row = 0
    while stones_in_row < max_stones_in_row:
      picture += "*"
      stones_in_row += 1
    picture += "\n"
    rows += 1
  while remainder > 0:
    picture += "*"
    remainder -= 1
  list_of_already_n_at_end = [1, 2, 6, 16]
  if stones_in_pile not in list_of_already_n_at_end:
    picture += "\n"
  if stones_in_pile == 1:
    return f"{picture}There is {stones_in_pile} stone in the pile."
  else: 
    return f"{picture}There are {stones_in_pile} stones in the pile."
...
def is_valid_move(stones_taken:int, stones_in_pile:int, valid_guesses:list) -> bool:
  """Determines whether a move is valid. A valid move must be included in valid_guesses, and cannot 
     cause a player to take the last stone.
      ...
      False
  """
  if stones_taken in valid_guesses and (stones_in_pile - stones_taken) > 0:
    return True
  else:
    return False
...
def get_ai_guess(stones_in_pile:int, valid_guesses:list[int]) -> int:
  """Picks the perfect number of stones to eventually leave the other player with 1 remaining stone
      ...
      1
  """
  remainder = stones_in_pile % (valid_guesses[-1]+1)
  if remainder == 1:
    return valid_guesses[0]
  elif remainder == 0:
    return valid_guesses[-1]
  else:
    return valid_guesses[remainder-2]
  # could use the parameter valid guesses instead of hard coding for 1-4
